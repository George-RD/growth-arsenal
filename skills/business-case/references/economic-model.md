...
Start the payback search at the first period containing an incremental economic burden; ignore any leading periods where both options have zero economics. If that first burden period aggregates cost and benefit and the cumulative incremental amount is already zero or positive, report immediate or first-period payback according to the model's period definition. If the balance is negative, payback is the first later period where it reaches zero or becomes positive. If there is no incremental burden, start at the first non-zero incremental period.
...
If the chosen counterfactual has zero relevant benefit, cost and quantified dis-benefit on the appraisal basis, and the tested driver changes only the candidate option, a shortcut is valid:
...
If both candidate and counterfactual options have different recovery and economic burdens but share the same affected-time and value-per-hour driver:
